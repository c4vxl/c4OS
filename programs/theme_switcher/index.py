...
def ask_for_pw(window_width=400, window_height=300):
    ...
    label = ctk.CTkLabel(root, text="Please enter your password", wraplength=250, font=("'Courier New', Courier, monospace", 16))
    label.place(relx=0.5, rely=0.1, anchor="center")

    input_entry = ctk.CTkEntry(root, width=window_width // 2, placeholder_text="Password", show="*")
    input_entry.place(relx=0.5, rely=0.3, anchor="center")

    error_label = ctk.CTkLabel(root, text="Incorrect! Please try again", wraplength=250, font=("'Courier New', Courier, monospace", 11), text_color="#c9a84f")

    def check_pw():
        try:
            subprocess.run(['su', getpass.getuser(), '-c', 'true'], input=input_entry.get().encode(), check=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
            password_holder[0] = input_entry.get()
            root.destroy()
    ...
    yes_btn = ctk.CTkButton(btn_frame, text="Confirm", command=check_pw)
    yes_btn.pack(pady=3)
    no_btn = ctk.CTkButton(btn_frame, text="Close", command=shutdown)
    no_btn.pack()

    root.mainloop()

    return password_holder[0]

def install_theme_github(github_usr, repo, ask_for_perms = True):
    cmd = f"sudo -Sv && bash <(curl -s https://raw.githubusercontent.com/{github_usr}/{repo}/refs/heads/main/installer.sh)"
    if ask_for_perms:
        cmd = f"yes '{ask_for_pw()}' | {cmd}"
